...
function AddPage($orientation='', $size=''){
	$tempPageNo=$this->page;
	if(count($this->pages)-1>$this->page){
		$this->page++;
	}else{
		$this->realAddPage($orientation, $size);
	}
}
function realAddPage($orientation='', $size='')
{
	// Start a new page
	if($this->state==0)
		$this->Open();
...
function _putinfo()
{
	$this->_out('/Producer '.$this->_textstring('outReport '.tFPDF_VERSION));
	if(!empty($this->title))
